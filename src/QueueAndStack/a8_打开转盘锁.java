    ...
    public static int openLock(String[] deadends, String target) {
        ...
        while (!queue.isEmpty()){
            ...
            for (int i = 0; i <size ; i++) {
                ...
                if(node.equals(target))return step;
                for (int j = 0; j <4 ; j++) {

                    char[] ch=node.toCharArray();
                    if(ch[j]=='9')ch[j]='0';
                    else ch[j]+=1;
                    String up=String.valueOf(ch);
                    if(!viewed.contains(up)){
                        queue.offer(up);
                        viewed.add(up);
                    }
                    ...
                    String down=String.valueOf(ch);
                    if(!viewed.contains(down)){
                        queue.offer(down);
                        viewed.add(down);
                    }
                }
