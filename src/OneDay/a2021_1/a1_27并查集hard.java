...
public class a1_27并查集hard {

    public static void main(String[] args) {
        int[][] a={{3,2,3},{1,1,2},{2,3,4}};
        System.out.println(maxNumEdgesToRemove(4,a));
    }
    ...
    public static int maxNumEdgesToRemove(int n, int[][] edges) {
        ...
        for (int i = 0; i <edges.length ; i++) {
            if(edges[i][0]==3){
                if(Alice.find(edges[i][1])==Alice.find(edges[i][2]))ans++;
                else {
        ...
        int countB=0;
        for (int i = 1; i <=n ; i++) {
            if(Alice.parent[i]==i)countA++;
            if(Bob.parent[i]==i)countB++;
        }
